#!/usr/bin/env bash
# Launch eval_two_stage_musan_fa.py once per GPU, then merge shard outputs.
#
# Usage:
#   bash scripts/eval_two_stage_musan_fa_shards.sh [HYDRA OVERRIDES...]
#
# Required overrides:
#   prep.output_dir=...
#
...
#
# Remaining arguments are passed to every eval_two_stage_musan_fa.py process.
...
for ((index=0; index<NUM_SHARDS; index++)); do
  ...
  CUDA_VISIBLE_DEVICES="${GPU_LIST[${index}]}" \
    "${PYTHON_BIN}" scripts/eval_two_stage_musan_fa.py \
      "${OVERRIDES[@]}" \
